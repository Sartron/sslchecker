...
readonly VERSIONDATE='September 4 2017';
readonly NUMBERREGEX='^[0-9]+$';
SNICOMP='1';
TIMEOUTCOMP='1';
HOSTISIP='0';
	...
	do
		case ${1} in
			...
			--nosni)
				...
				[ ${SNICOMP} == '0' ] && echo 'Server does not support SNI, no servername will be specified.';
				NOSNI='1';
				shift;
				;;
			--san)
...
function ParseStdin()
{
	...
	[ "${exitcode}" == '0' ] && X509_DisplayInfo "${stdin_x509}" '1' && exit 0;
	[ "${exitcode}" != '0' ] && echo -e 'Unable to interpret value from stdin. (error 7)\nVerify the certificate you are using is valid.' && exit 7;
}
##########################
...
function X509_DisplayInfo()
{
	...
	[ ${2} == '0' ] \
		...
		[ ${HOSTISIP} == '0' ] && echo " - $(dig ${HOST} +short | head -1)"
		)";
...
function SClient_ErrorParse()
{
	...
	if [[ $(echo "${1}" | grep -E ":errno=|:error:|didn't found starttls") ]]; then
		[ "$(echo "${1}" | grep 'socket: Connection refused')" ] && echo "Connection to ${HOST}:${PORT} timed out! (error 3)" && return 3;
		[ "$(echo "${1}" | grep 'socket: Connection timed out')" ] && echo "Connection to ${HOST}:${PORT} was refused! (error 2)" && return 2;
		[ "$(echo "${1}" | grep 'SSL23_GET_SERVER_HELLO:unknown protocol')" ] && \
...
function SClient_Connect()
{
	...
	local connectionstr=$(echo |$([ ${TIMEOUTCOMP} == '1' ] && echo " timeout ${TIMEOUT}") openssl s_client -connect "${1}:${2}"$([[ -n ${PROTOCOL} ]] && echo " -starttls ${PROTOCOL}")$([[ -n ${3} ]] && echo " -servername ${3}") -showcerts 2>&1);
...
function Main()
{
	...
	if [[ ${SNICOMP} == '1' && ${NOSNI} != '1' ]] && [[ ${HOSTISIP} != '1' ]] || [[ ${HOSTISIP} == '1' && -n ${NAME} ]]; then
		sclient_sni=$(SClient_Connect ${HOST} ${PORT} ${NAME}) || sclient_sni_exitcode=${?};
		[ ${sclient_sni_exitcode} ] && echo -e "${sclient_sni}" && return ${sclient_sni_exitcode};
		X509_DisplayInfo "${sclient_sni}" '0' && return 0;
	fi
	
	# No SNI.
	sclient=$(SClient_Connect ${HOST} ${PORT}) || local sclient_exitcode=${?};
	[ ${sclient_exitcode} ] && echo -e "${sclient}" && return ${sclient_exitcode};
...
if [ -n "${HOST}" ]; then
	if [ $(ping -c 1 -w 3 ${HOST} &> /dev/null; echo ${?}) != '0' ]; then
		...
		[ ${FORCE} == '1' ] && echo "${HOST} did not respond, but --force was passed." || echo -e "${HOST} did not respond! (error 5)\nTry passing --force if host does resolve.";
		[ ${FORCE} == '1' ] || exit 5;
	fi
